...
public final class StockDataInterface {
	...
	private static ArrayList<String> stocks; // list of stocks hard coded we can
												// choose from

	private static Date today; // date object for the stock request. This is for
								// future proofing when data and dates etc all
								// need to be fetched from the api

	private static Random rand; // random number generator to simulate the
								// prices of stocks

	// Constructor to initialise fields and add some stocks to the list we can
	// choose from
	...
	private static void setUp() {
		...
		stocks.add("GE"); // add General Electric to the list of hard coded
							// stocks
	...
	public static double getCurrentPrice() {
		...
		return rand.nextDouble() * 100; // return a random value between 0 and
										// 1, times 100
	}
	...
	public static boolean doesExist(String inStock) {
		...
		else { // otherwise the test must be false as the stock is not in our
				// list
			test = false;
		}
